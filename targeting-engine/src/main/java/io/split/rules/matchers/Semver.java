...
public class Semver {
    ...
    private String setAndRemoveMetadataIfExists(String version) {
        ...
        if (_metadata == null || _metadata.isEmpty()) {
            throw new IllegalArgumentException("Unable to convert to Semver, incorrect pre release data");
        }
    ...
    private String setAndRemovePreReleaseIfExists(String vWithoutMetadata) {
        ...
        _preRelease = preReleaseData.split(VALUE_DELIMITER_REGEX);
    ...
    private void setMajorMinorAndPatch(String version) {
        String[] vParts = version.split(VALUE_DELIMITER_REGEX);
        if (vParts.length != 3)
