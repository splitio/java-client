...
public class UserCustomSplitAdapterConsumerTest {
    ...
    public void setUp() throws NoSuchFieldException, IllegalAccessException {
        ...
        userCustomSplitAdapterConsumer.set(_userCustomSplitAdapterConsumer, userStorageWrapper);
    }

    @Test
    public void testGetChangeNumber() {
        Mockito.when(userStorageWrapper.get(PrefixAdapter.buildSplitChangeNumber())).thenReturn(getLongAsJson(120L));
    ...
    public void testGetSplit() {
        ...
        Mockito.when(userStorageWrapper.get(PrefixAdapter.buildSplitKey(SPLIT_NAME))).thenReturn(getSplitAsJson(split));
    ...
    public void testGetAll(){
        ...
        Mockito.when(userStorageWrapper.getKeysByPrefix(Mockito.anyObject())).
        ...
        Mockito.when(userStorageWrapper.getMany(Mockito.anyObject())).
        ...
        Mockito.verify(userStorageWrapper, Mockito.times(1)).getKeysByPrefix(Mockito.anyString());
        Mockito.verify(userStorageWrapper, Mockito.times(1)).getMany(Mockito.anyObject());
    }

    @Test
    public void testGetAllWithWrapperFailing(){
        Mockito.when(userStorageWrapper.get(PrefixAdapter.buildGetAllSplit())).
                thenReturn(null);
        List<ParsedSplit> splitsResult = (List<ParsedSplit>) _userCustomSplitAdapterConsumer.getAll();
        Assert.assertNotNull(splitsResult);
    ...
    public void testGetAllNullOnWrappers() {
        Mockito.when(userStorageWrapper.getKeysByPrefix(PrefixAdapter.buildGetAllSplit())).
                thenReturn(null);
        List<ParsedSplit> splitsResult = (List<ParsedSplit>) _userCustomSplitAdapterConsumer.getAll();
        Assert.assertEquals(0, splitsResult.size());
    }
    ...
    public void testFetchMany(){
        ...
        Mockito.when(userStorageWrapper.getItems(PrefixAdapter.buildFetchManySplits(Stream.of(SPLIT_NAME, SPLIT_NAME+"2").collect(Collectors.toList())))).
